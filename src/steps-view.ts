...
import { EventDispatchingStep, SimulationResult } from "./simulator";
    ...
    STEPS_SLIDER.setAttribute("max", String(steps.length - 1));
      ...
      STEPS_LOGS_CONTAINER.innerHTML = steps
        .map((step, index) => {
          return `
            <li>
              <strong>Current target:</strong> <code>${getTreeNodeLabel(step.currentTarget as TreeNode)}</code> | 
              <strong>Target:</strong> <code>${getTreeNodeLabel(step.target as TreeNode)}</code> |
              <strong>Composed Path:</strong> <code>${step.composedPath.map(node => getTreeNodeLabel(node as TreeNode)).join(', ')}</code>
            </li>
          `;
        })
        .join("\n");
